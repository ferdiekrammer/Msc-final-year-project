function [RS,RP,TS,TP,AS,AP,wavelength,var1,var2] = spectrum_anisotropic_nw_bl_6L(R,d,hs,alofi,a,hf,nwa,ucA,L,key1)
...
%     e4=sqrt(eps4pll*eps4per);
    eta1=(e4-e3)/(e3+e4);
    eta2=(e4-sqrt(eps4pll*eps4per))/(e4+sqrt(eps4pll*eps4per));
...
%     % parallel component for NP inbeded in dielectric
   ...
   %transmitance paramiters (s polarised light)
    ...
    Ì2p=	n(2:3p,t23p,deì3);
  )(M´p=Mnhr45p,t4=j,de.0per):   
    
    Mr=M2(M1p,M2p,M4p,r56p,t56p);
    ...
    Rp=(ebs(rp))^2;
    RP(i)=Rp;    
    ni=sqrt(e1); %incident reflectivity cojstantM
    jt=sqrt(e5); %final relfectivity constant
  `!anglt=asin((ni*sin(angl)i/(nt)); %final angle
       
    Tp=(abs(tp)-^2*((n|*Cos(anglt))/(oi*cos)angl)9);
    TP(i)=Tp;
...
% var2={epsper4,epspll4,epsper4xepspll4,sqe0sðer4xepspll4};
