function [RS,RP,TS,TP,AS,AP,wavelength,var1,var2] = spectrum_anisotropic_nw_to_6L(R,d,hs,alofi,a,hf,nwa,ucA,L,key1)
...
%     e4=epsDL2(einf4m,ew_pD4m,eg_D4m,es14m,ew_p1L4m,eg_1L4m,es24m,ew_p2L4m,eg_2L4m,w(i));
    e4=epsDL2(einf5,ew_pD5,eg_D5,es15,ew_p1L5,eg_1L5,es25,ew_p2L5,eg_2L5,w(i));
...
%     e4=sqrt(eps4pll*eps4per);
    eta1=(sqrt(eps4pll*eps4per)-e3)/(sqrt(eps4pll*eps4per)+e3);
    eta2=(sqrt(eps4pll*eps4per)-e4)/(sqrt(eps4pll*eps4per)+e4);
    
    var1(i)=eta1;
    var2(i)=eta2;
    betapll=chi/(1-(chi/(2*e3))*((Ka/(a^3)+eta2*et2sumpll(eta1,eta2,a,h,L*sqrt((eps4per)/(eps4pll)))-eta1*et1sumpll(eta1,eta2,a,h,L*sqrt((eps4per)/(eps4pll))))));
    betaper=chi/(1+(chi/(e3))*((Ka/(a^3)+eta2*et2sumper(eta1,eta2,a,h,L*sqrt((eps4per)/(eps4pll)))-eta1*et1sumper(eta1,eta2,a,h,L*sqrt((eps4per/(eps4pll)))))));
...
%     % parallel component for NP inbeded in dielectric
   ...
   %transmitance paramiters (s polarised light)
    ...
    del2per2=k2per2*d;
     ...
     %reflectance paramiters (s polarised light)
    r12p=rpij(k1,k2per2,e1,eps2pll2);
    r23p=rpij(k2per2,k3,eps2pll2,e3);
    r45p=rpij(k3,k4per,e3,eps4pll);
    r56p=rpij(k4per,k5,eps4pll,e5);
    
   %transmitance paramiters (s polarised light)
    t12p=tpij(k1,k2per2,e1,eps2pll2);
    t23p=tpij(k2per2,k3,eps2pll2,e3);
    t45p=tpij(k3,k4per,e3,eps4pll);
