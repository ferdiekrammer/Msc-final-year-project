...
for i=1:length(w)
    ...
    einf4d=epsSE(B1,B2,B3,C1,C2,C3,w(i));
    ...
    nt=sqrt((eps4per)); %final relfectivity constant
    ...
    Tp=(abs(tp))^2*((abs(nt)*cos(anglt))/(ni*cos(angl)));
    TP(i)=Tp;
end
...
axis(p2,[300 900 min(real(epspll4))-5 max(real(epspll4))+5])
