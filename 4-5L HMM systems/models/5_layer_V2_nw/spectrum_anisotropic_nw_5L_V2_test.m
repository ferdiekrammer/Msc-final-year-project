function [RS,RP,TS,TP,AS,AP,wavelength,var1,var2] = spectrum_anisotropic_nw_5L_V2_test(R,d,hs,alofi,a,hf,nwa,ucA,key1)
...
ep=zeros(1,length(w));
s4per_run=zeros(1,length(w));
s4pll_run=zeros(1,length(w));
k4per_run=zeros(1,length(w));
k4pll_run=zeros(1,length(w));
s4per_run_tp=zeros(1,length(w));
for i=1:length(w)
    ...
    s4per_run(i)=s4per;
    s4pll_run(i)=s4pll;
    s4per_run_tp(i)=2*sqrt(-1)*k4per*hf;
...
var2={epsper4,epspll4,epsper4xepspll4,sqepsper4xepspll4,ep,s4per_run,s4pll_run,k4per_run,k4pll_run,s4per_run_tp};
