function [RS,RP,TS,TP,AS,AP,wavelength,var1,var2] = spectrum_anisotropic_nw_5L_V2(R,d,hs,alofi,a,hf,nwa,ucA,key1)
...
%     einf4d=epsSE(B1,B2,B3,C1,C2,C3,w(i));
    ...
    eta1=-(e3-sqrt(eps4pll*eps4per))/(sqrt(eps4pll*eps4per)+e3);
    eta2=-(e5-sqrt(eps4pll*eps4per))/(e5+sqrt(eps4pll*eps4per));
    var1(i)=(eta1);
...
%     N=eta2*et2sumpll(eta1,eta2,a,h,hf,epll,eper)
%    M=eta1*et1sumpll(eta1,eta2,a,h,hf,epll,eper)
%    P=Ka/(a^3)+eta2*et2sumpll(eta1,eta2,a,h,hf,epll,eper)-eta1*et1sumpll(eta1,eta2,a,h,hf,epll,eper)

    
    betapll=chi/(1-(chi/(2*e3))*((Ka/(a^3)+eta2*et2sumpll(eta1,eta2,a,h,hf,epll,eper)-eta1*et1sumpll(eta1,eta2,a,h,hf,epll,eper))));
    betaper=chi/(1+(chi/(e3))*((Ka/(a^3)+eta2*et2sumper(eta1,eta2,a,h,hf,epll,eper)-eta1*et1sumper(eta1,eta2,a,h,hf,epll,eper))));
%     betapll=chi/(1-(chi/(2*e3))*((Ka/(a^3)+eta*((fn/a^3)-(3/2)*(g1n/(a^3))+1/(8*h^3)))))
