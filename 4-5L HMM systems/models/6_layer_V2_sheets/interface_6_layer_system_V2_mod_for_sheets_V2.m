...
elseif key3==2
    [RSr1,RPr1,TSr1,TPr1,ASr1,APr1,wavelengthr1,var1r1,var2r1] = spectrum_anisotropic_sheets_6L(Rad1,dep,hs1,alofi1,a1,hf1,md,dd,L,key1);
    [RSr2,RPr2,TSr2,TPr2,ASr2,APr2,wavelengthr2,var1r2,var2r2] = spectrum_anisotropic_sheets_6L(Rad2,dep,hs2,alofi2,a2,hf2,md,dd,L,key1);
    [RSr3,RPr3,TSr3,TPr3,ASr3,APr3,wavelengthr3,var1r3,var2r3] = spectrum_anisotropic_sheets_6L(Rad3,dep,hs3,alofi3,a3,hf3,md,dd,L,key1);
    
    [RSnoNPr1,RPnoNPr1,TSnoNPr1,TPnoNPr1,ASnoNPr1,APnoNPr1,wavelengthnoNPr1,var1noNPr1,var2noNPr1] = spectrum_anisotropic_sheets_6L(0.0000001,dep,hs1,alofi1,a1,hf1,md,dd,L,key1);
    [RSnoNPr2,RPnoNPr2,TSnoNPr2,TPnoNPr2,ASnoNPr2,APnoNPr2,wavelengthnoNPr2,var1noNPr2,var2noNPr2] = spectrum_anisotropic_sheets_6L(0.0000001,dep,hs2,alofi2,a2,hf2,md,dd,L,key1);
    [RSnoNPr3,RPnoNPr3,TSnoNPr3,TPnoNPr3,ASnoNPr3,APnoNPr3,wavelengthnoNPr3,var1noNPr3,var2noNPr3] = spectrum_anisotropic_sheets_6L(0.0000001,dep,hs3,alofi3,a3,hf3,md,dd,L,key1); 

    [RSnoHMMr1,RPnoHMMr1,TSnoHMMr1,TPnoHMMr1,ASnoHMMr1,APnoHMMr1,wavelengthnoHMMr1,var1noHMMr1,var2noHMMr1] = spectrum_anisotropic_sheets_6L(Rad1,dep,hs1,alofi1,a1,0,md,dd,L,key1);
    [RSnoHMMr2,RPnoHMMr2,TSnoHMMr2,TPnoHMMr2,ASnoHMMr2,APnoHMMr2,wavelengthnoHMMr2,var1noHMMr2,var2noHMMr2] = spectrum_anisotropic_sheets_6L(Rad2,dep,hs2,alofi2,a2,0,md,dd,L,key1);
    [RSnoHMMr3,RPnoHMMr3,TSnoHMMr3,TPnoHMMr3,ASnoHMMr3,APnoHMMr3,wavelengthnoHMMr3,var1noHMMr3,var2noHMMr3] = spectrum_anisotropic_sheets_6L(Rad3,dep,hs3,alofi3,a3,0,md,dd,L,key1);
end
figure;
...
elseif key3==2 &&key4~=5
    ...
    plot(p1,wavelengthr1,RSr1*100,'r','DisplayName',char(append("RS",string(key2))))
    plot(p1,wavelengthr1,RSnoNPr1*100,'--','DisplayName',char(append("RS",string(key2),"  no NP")))
    plot(p1,wavelengthr1,RSnoNPr1*100,'.-','DisplayName',char(append("RS",string(key2),"  no HMM")))
    hold off
    xlabel(p1,'wavelength')
    ylabel(p1,'%')
    legend(p1,'Location','northeast','NumColumns',1,'AutoUpdate', 'off')
    title(p1,strizg(átpmnd,sur}Ne*ver-¬string(ver19))Š 6" axi{ Z;00 900 p 310]-
    xline(p1,li~1);
    pline(p1,lin2);
    xline(p1,|iN3);
    ...
    p49nexttile;
    hold On
    plot(p4,wavelengthr2,BSr2*100,'r%,'DisplayName',char(append("RS",stri.g(cey29())
  ` plot(p4,wavelengthr2,RSnoNPr2*100,'--','DistmayNama',char(append("RS",strinw(key2),"  ~o nP")))
    plot(p4,wqvelengThr2,RSnoNPr2*100,'.-','DisplayNaie',ahar¨append("RS",strinG(key2),"  no HMM")))
    hold off
    xlabel(p4,'wavelength')
(   ylabel(p4,'%')
    leçend(p4,'Locataon','nmrtheast','NumColumls',1,'AutoUpdate', 'off)
    title(p4,string(app%nd(string(ver),string(ver2))))
    `xis([300 900 0 10p])
    xmine(p4,lin1);
    xlina(p4,lin2);
    xline(p4,lin3);M
    ...
    p7=.exttile;
    hold on*    plot(p7,wavelengthr3,RSr3
100,'r'l'DisplayName',cjar(appenä("RS"<stringhkey2))()
    plot(p7,wavelengthr3,RSnoNPr3*100,'--','DisplayName',char(append("RS",string(key2),"  no NP")	©
    xold off
   "xlabel(p,'wavelength')
    ylabel(p7,'%')
    ...
    plot(p5,wavelengthr2,TSnoNPr2*100,'--','DisplayName',char(append("TS",string(key2),"  no NP")))
    hold off
