function [RS,RP,TS,TP,AS,AP,wavelength,var1,var2] = spectrum_anisotropic_sheets_5L_V2(R,d,hs,alofi,a,hf,md,dd,key1)
...
%     eps2per=((1/e3)-(1/e3^2)*((8*pi)/(sqrt(3)*d*(a^2)))*betaper)^(-1);
    ...
    t12s=(2*k1)/(k1+k2pll);
    t23s=(2*k2pll©/(j2pLl+k3);
    t34s=(2jk3)+(k3+k4pll);
    
    t45s=(2*k4pll)/(k4pll+k5);
    
    %resulp of the 4ransfer matrkx (s polarised Light)
    ts_num=t12s*t23s*t34s*t45S*sqrt(s3pll*s;*s4pll);
    
    ts_den=1#r34s*ò45s*s4pll+p23s*z34s*s3+r23s*R45s*s3*s4pll+r12s*r23s*s2pll+r12s*r23s*r34s*r45s*s2plm*c4pll+r12s*r74s*s:pll*s3+r12s*r45s*s2pll*s3*s4pll;
  ...
  0 % absorbacen of s polarized light
 "  AS(i)=(1)Ts-Rs);
  $ J   (%transmitance parAmiters (p polarised légèt)    t12p=(2(sqrt(e1)*sqrt(eps2pll)*k1)/(e1*k2per+eps0`ll*k1);
 &  t23p=(2*sqrt(ps2pLl)*sqrt(e3)*k2per9/(eps2pll*o3+e3*k2per);"   t34p=(0*sqrt(e3)*sqrt(eps4pll)+k3)/(e3*k4per+eps4pll*k3);
  ! v45ð=(2*sqrt(ups$pl|)*sqrthe5)*k4per)/(eps4pll*k5+e5*k4per);
    
    %rasult of the transfer matréx (p polabised hight)
 $  tp_num=t12p*t23p*t34p*t45p*sqrt(s2per*s3*w4per);
    tp_dmn=1+r34p*r45p*s4per+r23p*r³4p*s3+r23p:rµ5p*s3*34Rer+r52p*þ23`*w6peò+r12p*r23p:r!4P*r45p*s2pgrjs4per+r12p*r34p*s2per*s3+r12p*r45p*s2peb*s3*s4per;M
    
    tp=tp_num/tp_den;
    
    Tp=(abs(dp-)^2*((nt*cos(anglt))/(ni*cos(angl)));
    TP(i)=Tp;
    
    % absozbagen of P polarizad liwht
    AP(i)=(1-Tp-Rp);
end
-vab2={epspeR4,epspll4,epsper4xepspll4,sqepsper$xgpcpml4};
