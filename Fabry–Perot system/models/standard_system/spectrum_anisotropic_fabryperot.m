function [RS,RP,TS,TP,AS,AP,wavelength,var1,var2] = spectrum_anisotropic_fabryperot(R,d,hs,alofi,a,hf,L,key1)
...
%     
    ...
    k8=kGen(eps1,eps8,angl,w(i));
    k9=kGen(eps1,eps9,angl,w(i));
    
    hs1=R+hs-d/2;
    
%     L-2*hs1-2*d+d+d+hs1+hs1
    %phase shifts
    del2=k2*hf;
    del3=k3*hs1;
    del4pll=k4pll*d;
...
%     M8s=Mn(r89s,t89s,0);
    ...
    r12p=rpij(k1,k2,eps1,eps2);
    r23p=rpij(k2,k3,eps2,eps3);
    ...
    r78p=rpij(k7,k8,eps7,eps8);
    r89p=rpij(k8,k9,eps8,eps9);
    
   %transmitance paramiters (s polarised light)
    t12p=tpij(k1,k2,eps1,eps2);
    t23p=tpij(k2,k3,eps2,eps3);
...
%     
%     r12p=rpij(k1,k2,eps1,eps2);
%     r23p=rpij(k2,k3,eps2,eps5);
% %     r34p=rpij(k3,k4per,eps3,eps4pll);
% %     r45p=rpij(k4per,k5,eps4pll,eps5);
% %     r56p=rpij(k5,k6per,eps5,eps6pll);
% %     r67p=rpij(k6per,k7,eps6pll,eps7);
%     r78p=rpij(k7,k8,eps5,eps8);
%     r89p=rpij(k8,k9,eps8,eps9);
%     
%    %transmitance paramiters (s polarised light)
%     t12p=tpij(k1,k2,eps1,eps2);
%     t23p=tpij(k2,k5,eps2,eps3);
% %     t34p=tpij(k3,k4per,eps3,eps4pll);
% %     t45p=tpij(k4per,k5,eps4pll,eps5);
% %     t56p=tpij(k5,k6per,eps5,eps6pll);
% %     t67p=tpij(k6per,k7,eps6pll,eps7);
%     t78p=tpij(k5,k8,eps7,eps8);
%     t89p=tpij(k8,k9,eps8,eps9);
%     
%     
%     %result of the transfer matrix (s polarised light)
%     M1p=Mn(r12p,t12p,del2);
%     M2p=Mn(r23p,t23p,del5);
% %     M3p=Mn(r34p,t34p,del4per);
% %     M4p=Mn(r45p,t45p,del5);
% %     M5p=Mn(r56p,t56p,del6per);
% %     M6p=Mn(r67p,t67p,del7);
